...
class ClienteService:
    ...
    def busca_cliente(self, cpf_cnpj):
        with self.conexao as con:
            cliente = con.session.query(Cliente).filter_by(cpf_cnpj=cpf_cnpj).first()
            return cliente

    def lista_clientes(self):
        with self.conexao as con:
            clientes = con.session.query(Cliente).filter(Cliente.ativo == True).all()
            return clientes
    ...
    def atualiza_cliente(self, cpf_cnpj, campo, valor):
        with self.conexao as con:
            try:
                ...
                if cliente:
                    if campo in campos:                    
                        con.session.query(Cliente).filter(Cliente.cpf_cnpj == cpf_cnpj).update({campo: valor})
                    else:
        ...
        with self.conexao as con:
            try:
                ...
                if cliente and cliente.ativo:
                    con.session.query(Cliente).filter(Cliente.cpf_cnpj == cpf_cnpj).update({'ativo': False})
                    con.session.commit()
                else:
                    raise RegistroNaoExistenteError(
                        'Cliente não foi encontrado'
